...
function UtilityBar() {
  return (
    <div className="hidden md:block bg-white text-[13px] font-medium text-draken-dark pt-3 pb-1">
      <div className="max-w-[1200px] mx-auto px-6 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <button className="flex items-center gap-1.5 hover:text-draken-orange transition-colors"><Globe /> English <ChevronDown /></button>
          <button className="flex items-center gap-1.5 hover:text-draken-orange transition-colors"><ContrastIcon /> High Contrast</button>
          <button className="flex items-center gap-1.5 hover:text-draken-orange transition-colors"><ChatIcon /> Customer Support</button>
          <button className="flex items-center gap-1.5 hover:text-draken-orange transition-colors"><UserIcon /> Contact Sales</button>
        </div>
        <div className="flex items-center gap-6">
          <button className="hover:text-draken-orange transition-colors"><SearchIcon /></button>
          <a href="#" className="hover:text-draken-orange transition-colors">Log in</a>
          <button className="flex items-center gap-1 hover:text-draken-orange transition-colors">About <ChevronDown /></button>
        </div>
      </div>
    </div>
  );
...
function MegaMenuProducts() {
  const products = [
    { name: 'Agentic CRM', desc: 'The AI-first single source of truth.', icon: '🧠', color: 'bg-orange-50' },
    { name: 'Marketing AI Hub', desc: 'Automate campaigns & lead gen.', icon: '🚀', color: 'bg-blue-50' },
    { name: 'Sales AI Hub', desc: 'Close deals faster with predictive insights.', icon: '📈', color: 'bg-green-50' },
    { name: 'Service AI Hub', desc: '24/7 autonomous customer support.', icon: '💬', color: 'bg-purple-50' },
  ];
  return (
    <div className="absolute top-full left-0 w-full bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 transform origin-top group-hover:scale-100 scale-95">
      <div className="max-w-[1000px] mx-auto px-6 py-10 flex gap-12">
        <div className="w-1/3">
          <h3 className="font-logo font-bold text-2xl text-draken-dark mb-2">Platform</h3>
          <p className="text-[14px] text-draken-muted mb-6 leading-relaxed">Everything you need to scale your go-to-market engine natively built with AI.</p>
          <a href="#" className="inline-flex items-center gap-2 font-bold text-draken-orange hover:text-draken-orangeHover transition-colors">Explore full platform <ArrowRight /></a>
        </div>
        <div className="w-2/3 grid grid-cols-2 gap-6">
          {products.map((p) => (
            <div key={p.name} className="flex gap-4 p-4 rounded-xl hover:bg-gray-50 transition-colors cursor-pointer border border-transparent hover:border-gray-100">
              <div className={`w-12 h-12 rounded-lg ${p.color} flex items-center justify-center text-2xl shrink-0`}>{p.icon}</div>
              <div>
                <h4 className="font-bold text-draken-dark mb-1">{p.name}</h4>
                <p className="text-[13px] text-draken-muted leading-relaxed">{p.desc}</p>
              </div>
            </div>
          ))}
        </div>
...
function MegaMenuSolutions() {
  const solutions = {
    'By Role': ['Marketing Leaders', 'Sales Directors', 'Customer Success', 'Operations'],
    'By Industry': ['SaaS & Tech', 'E-commerce', 'Financial Services', 'Healthcare'],
    'By Need': ['Lead Generation', 'Revenue Operations', 'Customer Retention', 'Workflow Automation']
  };
  return (
    <div className="absolute top-full left-0 w-full bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 transform origin-top group-hover:scale-100 scale-95">
      <div className="max-w-[1000px] mx-auto px-6 py-10">
        <div className="grid grid-cols-3 gap-12">
          {Object.entries(solutions).map(([title, items]) => (
            <div key={title}>
              <h4 className="font-bold text-sm text-draken-dark uppercase tracking-wider mb-5 pb-3 border-b border-gray-100">{title}</h4>
              <ul className="space-y-4">
                {items.map((item) => (
                  <li key={item}>
                    <a href="#" className="group flex items-center justify-between text-[14px] font-medium text-draken-muted hover:text-draken-orange transition-all">
                      {item}
                      <span className="opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all text-draken-orange"><ArrowRight /></span>
                    </a>
                  </li>
                ))}
              </ul>
...
function MegaMenuResources() {
  const resources = [
    { title: 'Documentation', desc: 'Guides, API refs, and setup.', icon: '📚' },
    { title: 'Community', desc: 'Connect with 10k+ creators.', icon: '🌐' },
    { title: 'Blog', desc: 'Latest insights and product news.', icon: '✍️' },
    { title: 'Draken Academy', desc: 'Free certifications & courses.', icon: '🎓' },
  ];
  return (
    <div className="absolute top-full left-0 w-full bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 transform origin-top group-hover:scale-100 scale-95">
      <div className="max-w-[1000px] mx-auto px-6 py-10 flex gap-12">
          ...
          {resources.map((r) => (
            <div key={r.title} className="group cursor-pointer">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h4 className="font-bold text-draken-dark group-hover:text-draken-orange transition-colors">{r.title}</h4>
              </div>
              <p className="text-[13px] text-draken-muted pl-8">{r.desc}</p>
            </div>
          ))}
        </div>
        <div className="w-1/3 bg-gray-50 rounded-2xl p-6 border border-gray-100 relative overflow-hidden">
           <div className="absolute -top-4 -right-4 text-draken-orange/10 text-9xl">✨</div>
           <h4 className="font-bold text-draken-dark mb-2 relative z-10">New: Agentic Workflows</h4>
           <p className="text-[13px] text-draken-muted mb-4 relative z-10 leading-relaxed">Discover how our new AI agents can automate your entire sales pipeline in 5 minutes.</p>
           <a href="#" className="inline-flex items-center gap-1.5 text-sm font-bold text-draken-orange hover:text-draken-orangeHover transition-colors relative z-10">Read the guide <ArrowRight /></a>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <nav className={`sticky top-0 z-50 bg-white transition-shadow duration-300 ${scrolled ? 'shadow-md border-b-0' : 'shadow-none border-b border-gray-200'}`}>
      <div className={`max-w-[1200px] mx-auto px-6 flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-16' : 'h-[88px]'}`}>
        ...
        {/* mobile */}
        <button className="lg:hidden text-draken-dark" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Menu">
      ...
      {mobileOpen && (
        <div className="lg:hidden border-t border-gray-100 bg-white px-6 py-4 space-y-3 animate-fade-in">
          {['Products', 'Solutions', 'Pricing', 'Resources'].map((l) => (
            <a key={l} href={`#${l.toLowerCase()}`} className="block py-2 text-[15px] font-medium text-draken-dark" onClick={() => setMobileOpen(false)}>{l}</a>
          ))}
    ...
    <section className="relative min-h-[calc(100vh-5.75rem)] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-6 py-32 max-w-4xl mx-auto">
        <p className="text-white/90 text-sm font-semibold tracking-[0.25em] uppercase mb-8">
          Draken Agentic Customer Platform
        </p>
        <h1 className="font-serif text-white text-4xl sm:text-5xl md:text-6xl lg:text-[5rem] font-normal leading-[1.1] mb-8">
          Where go-to-market<br className="hidden sm:block" /> teams go to scale<span className="text-draken-orange">.</span>
        </h1>
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          ...
          <a href="#" className="btn-outline-orange !bg-white !text-draken-orange px-10 py-4 text-base">Get started free</a>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section className="py-10 bg-white border-b border-gray-100">
      <div className="max-w-[1100px] mx-auto px-6">
        <p className="text-center text-draken-muted text-base mb-8">
          <span className="font-semibold text-draken-dark">299,000+</span> customers in over <span className="font-semibold text-draken-dark">135</span> countries grow their businesses with Draken.
        </p>
...
function PlatformSection() {
  ...
  const slides = [
    {
      ...
      desc: "Draken's Smart CRM is the single source of truth that connects all your business data.",
      cards: [
        { label: 'Smart CRM', color: 'bg-draken-orange', icons: ['📊', '📈', '🔗', '⚡'] },
    ...
    },
    {
      ...
      cards: [
        { label: 'Marketing', color: 'bg-draken-peach' },
    ...
    },
    {
      ...
      cards: [
        ...
        { label: 'Agents', color: 'bg-draken-peach' },
  ...
  return (
    <section className="py-24 md:py-32 bg-draken-cream">
      <div className="max-w-[1100px] mx-auto px-6">
        <div className="text-center mb-16 animate-on-scroll">
          <h2 className="font-serif text-draken-dark text-4xl md:text-[3.5rem] leading-tight mb-4">
            Draken's Agentic Customer Platform
          </h2>
          <p className="text-draken-muted text-base md:text-lg max-w-2xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center animate-on-scroll">
          <div>
            <h3 className="font-serif text-draken-dark text-3xl md:text-[2.5rem] leading-snug mb-4">{s.title}</h3>
            <p className="text-draken-muted text-base md:text-lg">{s.desc}</p>
          </div>
          ...
          <div className="flex items-end justify-center gap-3 h-64 perspective-[800px]">
            {s.cards.map((c, i) => (
              ...
              >
                ...
                <span className="text-xs font-semibold text-draken-dark mt-2">{c.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center justify-center gap-6 mt-10">
          <button onClick={() => setActiveSlide((p) => (p - 1 + slides.length) % slides.length)} className="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-400 hover:border-draken-dark hover:text-draken-dark transition-colors"><ChevronLeft /></button>
          <div className="flex gap-2">
            {slides.map((_, i) => (
              <button key={i} onClick={() => setActiveSlide(i)} className={`w-2.5 h-2.5 rounded-full transition-colors ${activeSlide === i ? 'bg-draken-dark' : 'bg-gray-300'}`} />
            ))}
          </div>
          <button onClick={() => setActiveSlide((p) => (p + 1) % slides.length)} className="w-8 h-8 rounded-full border border-gray-300 flex items-center justify-center text-gray-400 hover:border-draken-dark hover:text-draken-dark transition-colors"><ChevronRight /></button>
        </div>
      </div>
    </section>
  );
      ...
      <div className="max-w-[1100px] mx-auto px-6">
        <div className="grid md:grid-cols-[1fr_2fr] gap-12 items-start">
          ...
          <div className="animate-on-scroll md:sticky md:top-28">
            <p className="text-xs font-semibold text-draken-muted mb-3 flex items-center gap-1.5">
              <span className="text-draken-orange">✦</span> Powered by AI
            </p>
            <h2 className="font-serif text-draken-dark text-3xl md:text-[2.8rem] leading-tight mb-6">
              Growing a business is hard. Draken makes it easier.
            </h2>
            <p className="text-draken-muted text-base leading-relaxed mb-8">
              Disconnected tools and data slow you down. Draken connects everything — and everyone — in one place to make growing a business easier than you think.
            </p>
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 group">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <h3 className="font-bold text-base text-draken-dark">
          {hub.name}
          {hub.tag && <sup className="text-[11px] text-draken-muted ml-0.5">{hub.tag}</sup>}
        </h3>
      </div>
      <ul className="space-y-3 mb-6">
        {hub.points.map((p, i) => (
          <li key={i} className="flex items-start gap-2.5 text-sm text-draken-muted leading-relaxed">
            <Check className="text-draken-orange mt-0.5 flex-shrink-0" />
            {p}
          </li>
        ))}
      </ul>
      <a href="#" className="inline-flex items-center gap-1.5 text-sm font-semibold text-draken-dark hover:text-draken-orange transition-colors group-hover:gap-2.5 pt-4 border-t border-gray-100 border-dashed w-full">
      ...
      <div className="max-w-[1100px] mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-16 animate-on-scroll">
          <div className="md:max-w-md">
            <span className="text-draken-orange text-2xl mb-3 block">✦</span>
            <h2 className="font-serif text-draken-dark text-3xl md:text-[2.8rem] leading-tight mb-4">
          ...
          <div className="md:max-w-sm md:text-right flex flex-col items-start md:items-end gap-4">
            ...
            <p className="text-draken-dark/70 text-base leading-relaxed">
              <span className="font-semibold text-draken-dark">Breeze Agents</span> are your always-on teammates. They can resolve over 65% of customer inquiries, accelerate your sales pipeline, and whip up quality content in no time.
            </p>
          </div>
        </div>
        ...
        <div className="relative flex items-center justify-center mb-8 animate-on-scroll group">
          <button 
            className="absolute left-0 md:left-12 z-20 w-10 h-10 bg-white rounded-full shadow-md flex items-center justify-center text-draken-dark hover:scale-110 transition-transform"
          ...
          <div className="flex items-end justify-center gap-4 md:gap-6 overflow-x-auto pb-4 w-full px-12 md:px-24">
            {agents.map((a, i) => (
              ...
              >
                {/* card mock content */}
                <div className={`rounded-xl p-3 mb-3 ${activeAgent === i ? 'bg-draken-orangeLight' : 'bg-gray-50'}`}>
                  {a.card.title && <p className="text-xs font-semibold text-draken-dark mb-1">{a.card.title}</p>}
                  <p className="text-[10px] text-draken-muted leading-relaxed">{a.card.text || a.card.user}</p>
                  {a.card.btn && (
                    <button className={`mt-2 text-[10px] font-semibold px-3 py-1 rounded ${activeAgent === i ? 'bg-draken-orange text-white' : 'bg-gray-200 text-draken-dark'}`}>
                      {a.card.btn}
                    </button>
                  )}
                </div>
                <h4 className="font-bold text-sm text-draken-dark">{a.name}</h4>
                <p className="text-[11px] text-draken-muted leading-relaxed mt-1">{a.desc}</p>
                {activeAgent === i && (
                  <a href="#" className="inline-flex items-center gap-1 text-[11px] font-bold text-draken-dark mt-2 underline underline-offset-2 decoration-draken-dark">
                    Learn more
                  </a>
                )}
              </div>
          ...
          <button 
            className="absolute right-0 md:right-12 z-20 w-10 h-10 bg-white rounded-full shadow-md flex items-center justify-center text-draken-dark hover:scale-110 transition-transform"
        ...
        <div className="flex justify-center mt-2">
          <button 
            ...
            className="w-8 h-8 rounded-full bg-white/60 hover:bg-white shadow-sm flex items-center justify-center text-draken-dark text-[10px] transition-colors"
          >
            {isPaused ? '▶' : '❚❚'}
          </button>
        </div>
  ...
  return (
    <section className="py-20 md:py-28 bg-white border-t border-gray-100">
      <div className="max-w-[1100px] mx-auto px-6">
        <div className="grid md:grid-cols-2 gap-12 items-center animate-on-scroll">
          <div>
            <h2 className="font-serif text-draken-dark text-3xl md:text-[2.5rem] leading-tight mb-4">
              Works with the tools you already use.<br />
              <span className="text-draken-orange font-semibold">2,000+</span> integrations.
            </h2>
            <a href="#" className="inline-flex items-center gap-1 text-base font-semibold text-draken-dark underline underline-offset-4 decoration-draken-dark hover:text-draken-orange transition-colors mt-3">
              See all app integrations
            </a>
          </div>
          <div className="flex flex-wrap gap-3 justify-center md:justify-end">
            {tools.map((t) => (
              <div key={t} className="bg-white border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-medium text-draken-dark shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-default">
                {t}
              </div>
            ))}
          </div>
...
function CaseStudies() {
  ...
  const studies = {
    Enterprise: {
      quote: '"Draken took the time to understand our business needs fully. The pre-sales and subsequent support really stood out from the start. They committed to engage with us deeply and work side-by-side with us on the implementation. They\'ve since more than met this commitment."',
      author: 'Adam Wijaya',
    ...
    'Mid-Sized Business': {
      quote: '"Since switching to Draken, our sales cycle has shortened by 40% and our team collaboration has improved dramatically. The platform is incredibly intuitive."',
      author: 'Sari Dewi',
    ...
    'Small Business': {
      quote: '"As a small business, every tool matters. Draken gave us enterprise-grade capabilities at a price we could afford. Our customer retention is up 25%."',
    ...
    <section className="py-24 md:py-32 bg-white">
      <div className="max-w-[1100px] mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 mb-8 animate-on-scroll">
          <div>
            <span className="text-xs font-semibold text-draken-muted border border-gray-300 px-3 py-1 rounded-full">Case Studies</span>
            <h2 className="font-serif text-draken-dark text-3xl md:text-[2.8rem] leading-tight mt-4">
          ...
          <div className="flex flex-col items-start md:items-end gap-3">
            ...
            <p className="text-draken-muted text-base max-w-sm md:text-right">Scale your business with Draken. The proof is in our customers' success.</p>
          </div>
        </div>
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`px-6 py-3.5 text-base font-medium transition-colors border-b-2 -mb-px ${
                tab === t
                  ? 'border-draken-dark text-draken-dark'
                  : 'border-transparent text-draken-muted hover:text-draken-dark'
              }`}
            >
              {t}
            </button>
        ...
        {/* case study card */}
        <div className="bg-draken-cream rounded-3xl p-8 md:p-12 animate-on-scroll">
          <div className="grid md:grid-cols-2 gap-10 items-center">
            ...
            <div>
              <p className="text-draken-dark/70 text-base md:text-lg leading-relaxed italic mb-6">
                {current.quote}
              </p>
              <p className="font-bold text-base text-draken-dark">{current.author}</p>
              <p className="text-xs text-draken-muted">{current.role}</p>
              <a href="#" className="inline-flex items-center gap-1 text-base font-bold text-draken-dark underline underline-offset-4 decoration-draken-dark mt-3 hover:text-draken-orange transition-colors">
          ...
          <div className="grid grid-cols-2 gap-8 mt-10 pt-8 border-t border-gray-200">
            {current.stats.map((s, i) => (
              <div key={i} className="text-center">
                <div className="font-serif text-5xl md:text-6xl text-draken-dark mb-2">
                  <Counter target={s.value} />
                </div>
                <p className="text-draken-muted text-sm md:text-base">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    <section className="py-16 md:py-20 bg-[#003A3A] text-white">
      <div className="max-w-[1100px] mx-auto px-6">
        <h2 className="font-serif text-4xl md:text-[3rem] leading-tight mb-10">
          Make impossible growth feel impossibly easy,<br /> with Draken<span className="text-draken-orange">.</span>
        </h2>
...
function Footer() {
  const columns = {
    ...
    'Customers': ['Customer Support', 'Join a Local User Group', 'Developer Tools', 'Draken Community'],
    'Partners': ['App Marketplace', 'Solutions Directory', 'Affiliate Program', 'Become a Partner'],
  };

  return (
    <footer className="bg-draken-dark text-white">
